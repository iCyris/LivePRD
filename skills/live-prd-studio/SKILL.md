...
- Treat the source PRD file as canonical and keep named working versions as separate Markdown files under a local version directory.
...
- Use static export for sharing and a local authoring server for saving named Markdown versions, restoring them, archiving them, and exporting the current PRD or demo bundle.
...
- Before deeper setup or troubleshooting, prefer running `npm run doctor` or `node ./packages/cli/index.mjs doctor` to check runtime, directories, dependencies, render validation, and local version storage.
...
- When the user asks to save a version, prefer a human-readable custom version name and write it as a local Markdown version file rather than only keeping it in browser state.
